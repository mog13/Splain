    ...
    processContexts(entry, context) {
        if (context && context.contexts) {
            let contextualEntry = entry.filter(value => {
                return value.hasOwnProperty("context") &&
                    (Array.isArray(value.context) && context.contexts.some(con => value.context.includes(con)) || context.contexts.includes(value.context));
            });

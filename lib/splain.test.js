...
describe("when i use splain ", () => {
    ...
    describe("it should handle a mismatched template token count", ()=> {

        it("should handle unclosed template",()=>{
            Splain.dictionary.addEntry({test1:["a"], test2:["b"], test3:["{{test4}}"], test4:["{{fake}}"]});
            expect(function() {Splain.process("it {{test1 {{test2}} {{test1 test2 test4 {{test3 test4}}}}");}).toThrow("Error: not enough closing tokens found in it {{test1 {{test2}} {{test1 test2 test4 {{test3 test4}}}}");
        });

        it("should handle unopened template",()=>{
            Splain.dictionary.addEntry({test1:["a"], test2:["b"], test3:["{{test4}}"], test4:["{{fake}}"]});
            expect(function() {Splain.process("it test1 test2}} {{test1 test2 test4 {{test3 test4}}}}}");}).toThrow("Error: not enough opening tokens found in it test1 test2}} {{test1 test2 test4 {{test3 test4}}}}}");
        });
    });
    ...
    describe("when running with a dictionary context", () => {
        it("should respect the dictionary context used",()=> {
            Splain.addEntry({greetings:["hello"]}, null, "EN");
            Splain.addEntry({greetings:["bonjour"]}, null,"FR");
            expect(Splain.process("{{greetings}}", null, "EN")).toBe("hello");
            expect(Splain.process("{{greetings}}", null, "FR")).toBe("bonjour");
        });

        it("should respect the dictionary context used with given name",()=> {
            Splain.addEntry(["hello"], "hellos", "EN");
            Splain.addEntry(["bonjour"], "hellos","FR");
            expect(Splain.process("{{hellos}}", null, "EN")).toBe("hello");
            expect(Splain.process("{{hellos}}", null, "FR")).toBe("bonjour");
        });
    });
    ...
    it("should compile templates in order from left to right",() => {
        Splain.addEntry({a:[{value: "A", context: "letter"}], b:[{value: "B", context: "letter"}],1:[{value: "one", context: "number"}],
            either:[{value: "D", context: "letter"},{value: "2", context: "two"}]});
        expect(Splain.process("{{a {{either}} b}}")).toBe("A D B");
    });

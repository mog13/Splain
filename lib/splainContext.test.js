...
describe("with splain context", ()=> {
    ...
    describe("when interacting with the cache", () => {
    ...
    describe("when resolving templates", function() {
        ...
        it("should set second resolution into array of values", () => {
            ...
            splainContext.addTemplateResolution("template", "resolution2");
            ...
            expect(splainContext.templateResolutions["template"][1]).toBe("resolution2");
        });
    });

    describe("when adding a context using a single value", () => {
        beforeEach(() => {
            splainContext.addContext("context");
        });

        it("should add the value to the context", () => {
            expect(splainContext.contexts.length).toBe(1);
            expect(splainContext.contexts[0]).toBe("context");
        });
    });

    describe("when adding a context using an array of values", () => {
        beforeEach(() => {
            splainContext.addContext(["context", "othercontext"]);
        });

        it("should add the value to the context", () => {
            expect(splainContext.contexts.length).toBe(2);
        });
    });
